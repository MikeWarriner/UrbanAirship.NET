...
namespace RestSharp
{
	...
	public partial class RestClient : IRestClient
	{
		...
		public RestClient()
		{
			...
			AddHandler("application/json", new JsonDeserializer(), true);
            AddHandler("application/xml", new XmlDeserializer(), true);
            AddHandler("text/json", new JsonDeserializer(), true);
            AddHandler("text/x-json", new JsonDeserializer(), true);
            AddHandler("text/javascript", new JsonDeserializer(), true);
            AddHandler("text/xml", new XmlDeserializer(), true);
            AddHandler("*", new XmlDeserializer(), true);

			// silverlight friendly way to get current vers ion
            ...
            //if (Environment.MachineName.ToLower() == "mwx220")
            //    Proxy = new WebProxy("192.168.2.10", 8080);
		...
		public void AddHandler(string contentType, IDeserializer deserializer, bool addToAccept)
		{
            
			ContentHandlers[contentType] = deserializer;
			if (contentType != "*" && addToAccept)
			{
        ...
        // NOTE : DO NOT CALL, SHOULD BE PRIVATE! (mfw for debugging)
		...
		private void ConfigureHttp(RestRequest request, IHttp http)
		{
			...
			if(body != null)
			{
                ...
                {
                    http.RequestBytes = (byte[])body.Value;
                }
                else
                {
